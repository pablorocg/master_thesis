...
class Multimodal_Text_Graph_Model(pl.LightningModule):
    ...
    def __init__(self, 
        ...
        self.label_templates = ["left arcuate fasciculus",
        ...
        self.accuracy_metric = torchmetrics.Accuracy(num_classes=8, task='multiclass')
        self.f1_metric = torchmetrics.F1Score(num_classes=8, average='weighted', task='multiclass')
        self.precision_metric = torchmetrics.Precision(num_classes=8, average='weighted', task='multiclass')
        self.recall_metric = torchmetrics.Recall(num_classes=8, average='weighted', task='multiclass')
        self.confusion_matrix_metric = torchmetrics.ConfusionMatrix(num_classes=8)
        self.roc_auc_metric = torchmetrics.ROC(num_classes=8, task='multiclass')
        self.pr_auc_metric = torchmetrics.PrecisionRecall(num_classes=8, task='multiclass')
        self.average_precision_metric = torchmetrics.AveragePrecision(num_classes=8, task='multiclass')
        self.auc_metric = torchmetrics.AUROC(num_classes=8, task='multiclass')
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Clasificar los logits en una de las posibles etiquetas
        graph, text = batch
        accuracy, f1 = self.clasificar(self.label_templates, graph)
        self.log('val_accuracy', accuracy, batch_size=128)
        self.log('val_f1', f1, batch_size=128)
        return loss
    ...
    def clasificar(self, posibles_text_labels, graph_batch):
        """
        Clasifica los logits en una de las posibles etiquetas.
        """
        tokenizer = AutoTokenizer.from_pretrained('distilbert-base-uncased')
    
        # Tokenizar las posibles etiquetas y mover al dispositivo correcto
        tokenized_text_labels = tokenizer(posibles_text_labels, padding=True, truncation=True, return_tensors="pt")
        tokenized_text_labels = {k: v.to(self.device) for k, v in tokenized_text_labels.items()}

        # Los siguientes pasos ya deberían manejar correctamente el dispositivo si los modelos están en self.device
        text_projections = self.text_encoder(tokenized_text_labels)
        text_projections = self.text_projection_head(text_projections)

        # Asegurar que graph_batch también se mueva al dispositivo correcto
        graph_projections = self.graph_encoder(graph_batch.to(self.device))
        graph_projections = self.graph_projection_head(graph_projections)

        #Calcular la similitud entre las posibles etiquetas y el grafo
        ...
        #Seleccionar la etiqueta con mayor similitud
        max_similarities, max_indices = dot_similarity.max(dim=0)
        # Obtener las etiquetas reales de los grafos (graph.y)
        real_labels = graph_batch.y
        # Obtener las etiquetas predichas (numero de la etiqueta)
        predicted_labels = max_indices
        # Obtener la accuracy (utilizar torchmetrics.Accuracy para calcularla)
        accuracy = self.accuracy_metric(predicted_labels, real_labels)
        # Obtener la F1 score (utilizar torchmetrics.F1 para calcularla)
        f1 = self.f1_metric(predicted_labels, real_labels)
        return accuracy, f1
...
class RandomTextGraphDataset(Dataset):
    def __init__(self, root="", transform=None, pre_transform=None, num_samples=4000, num_max_nodes=30, num_node_features=3, return_tokenized_text=False):
...
if __name__ == "__main__":
    ...
    dataset = RandomTextGraphDataset(num_samples=50000, return_tokenized_text=True)
    datamodule = TextGraphDataModule(RandomTextGraphDataset, batch_size=128, num_workers=4)


    model = Multimodal_Text_Graph_Model()
    trainer = Trainer(max_epochs = 1000, logger=TensorBoardLogger(r'C:\Users\pablo\GitHub\tfm_prg\lightning_logs'))#, fast_dev_run=50, callbacks=[DeviceStatsMonitor()]
