...
cfg = CFG()
if log:
    writer = SummaryWriter(log_dir=f"runs/graph_classif_v1_{cfg.dataset}_embeddings", filename_suffix=f"{time.time()}")
    run["config"] = {
        "dataset": cfg.dataset,
        "seed": cfg.seed,
...
for epoch in range(cfg.max_epochs):
    # random.shuffle(train_data)# Barajar los datos de entrenamiento
    ...
    for idx_suj, subject in enumerate(train_data):# Iterar sobre los sujetos de entrenamiento
        ...
        for i, graph_batch in enumerate(prog_bar):
            ...
            # Forward pass
            _, preds = model(graph_batch)# Batch size necesario para calcular la media de los embeddings
            

            # Calcular la pérdida
            loss = criterion(preds, graph_batch.y)
            ...
            targets = graph_batch.y#torch.cat((graph_anch.y, graph_pos.y, graph_neg.y))
    ...
    checkpoint_name = f'checkpoint_graph_classif_{cfg.dataset}_{cfg.encoder}_{cfg.embedding_projection_dim}_{epoch}.pth'
