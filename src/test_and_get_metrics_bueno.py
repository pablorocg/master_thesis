...
def get_dice_metrics_v2(trk_files, predictions, handler) -> tuple[float, float]:
    ...
    # Inicializa las estructuras antes del bucle
    ...
    # Itera sobre los archivos de tractos
    for tract in trk_files:
        # Obtener la etiqueta numérica del tracto
        ...
        # Diccinario con los streamlines de cada tracto por etiqueta
        streamlines_by_tract[tract_label] = streamlines

        # Añadir los streamlines al conjunto de todos los streamlines
        all_streamlines.extend(streamlines_by_tract[tract_label])

        # Añadir la etiqueta de los streamlines al conjunto de todas las etiquetas
        all_labels.extend([tract_label] * len(streamlines_by_tract[tract_label]))

        del tractography_data

    wDICE, DICE = [], []
    # Iterar sobre el diccionario de streamlines por tracto
    for tract_label, streamlines in streamlines_by_tract.items():
        # Obtener los indices de los streamlines predichos como tract_label
        predicted_as_c = [i for i in range(len(predictions)) if predictions[i] == tract_label]
        
        # Obtener los streamlines predichos como tract_label
        predicted_streamlines = ArraySequence([all_streamlines[i] for i in predicted_as_c])
        
        # Calcular las métricas de DICE y wDICE
        print(f'Fibras clasificadas como correctas para el tracto {tract_label}: {len(predicted_streamlines)}')
        print(f'Fibras totales para el tracto {tract_label}: {len(streamlines)}')
        ...
        # results_dict[tract_label] = (wdice.item(), dice.item())
        # devolver dos listas con los valores de wDICE y DICE para cada tracto
...
class CFG:
    def __init__(self):
        ...
        self.encoder = "GCNEncoder_supcon_finetuned" # Las opciones son "GAT" o "GCN" o "HGPSL"
        ...
        if self.dataset == "HCP_105":
            ...
            self.pretrained_model_path = "/app/trained_models/checkpoint_HCP_105_finetuned_GCNEncoder_v2_7_0.85.pth"
            self.n_classes = 72 # 72 tractos o 71 tractos sin CC

        elif self.dataset == "HCP_105_without_CC":
            self.ds_path = "/app/dataset/HCP_105"
            self.pretrained_model_path = "/app/trained_models/checkpoint_HCP_105_without_CC_finetuned_GCN_0_3.61.pth"
            self.n_classes = 71
...
classifier_model.load_state_dict(torch.load('/app/trained_models/checkpoint_HCP_105_without_CC_finetuned_GCN_0_3.61.pth')['model_state_dict'])
#===============================================================================
...
df = pd.DataFrame(columns = ['subject_id', 'tract', 'n_streamlines', 'accuracy', 'precision', 'recall', 'F1', 'AUCROC', 'DICE', 'wDICE'])
...
for idx_val, subject in tqdm(enumerate(test_data), total=len(test_data), desc="Subjects"):
    ...
    # Calcular precisión, recall, F1-score por cada clase
    # accuracy = [accuracy_score(targets, class_preds) for _ in range(len(set(targets)))]
    accuracy = []
    precision = []
    recall = []
    f1 = []
...
model_weights_filename = pathlib.Path(cfg.pretrained_model_path).stem
df.to_csv(f"/app/resultados/results_{model_weights_filename}.csv", index=False)
